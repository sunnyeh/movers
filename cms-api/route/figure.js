	...
	app.get('/figure',authCheck.ensureAuthorized,function(req,res,next){
		Sync(function(){
			try{
				...
				var sqlCount = "select count(id) as numRows from figure where not status=-1"+SQLFilter;
				...
				var sqlList = "select * from figure where not status = -1"+SQLFilter+" order by name limit "+SQLlimit;
		...
		Sync(function(){
			try{
				...
				if(id){
					...
					var sqlQuery = "insert into figure (name, figures, symbol, icon, image, sort_order, status, inserted_on, inserted_by, ip) values ('"+figure.name+"', '"+figure.figures+"', '"+figure.symbol+"', '"+figure.icon+"', '"+figure.image+"', '"+figure.sort_order+"', '"+figure.status+"', now(), '"+figure.inserted_by+"', '"+figure.ip+"')";
					console.log("sqlQuery",sqlQuery);
					var tempFigure = db.executeSql.future(null,sqlQuery);
