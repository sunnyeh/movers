	...
	app.post('/address',authCheck.ensureAuthorized,function(req,res,next){
		Sync(function(){
			try{
				...
				if(id){
					var tempAddress = db.executeSql.future(null,"update address set name='"+address.name+"',address='"+address.address+"', city='"+address.city+"', state='"+address.state+"', country='"+address.country+"', phone_no='"+address.phone_no+"', email='"+address.email+"', geo_long='"+address.geo_long+"', geo_lat='"+address.geo_lat+"', sort_order='"+address.sort_order+"', status='"+address.status+"', updated_by='"+address.updated_by+"', ip='"+address.ip+"', updated_on=now() where id = '"+id+"'")
					httpMsg.sendJson(req,res,{status:true,message:"successfully updated"})
				}else{
					var tempAddress = db.executeSql.future(null,"insert into address (name,address, city, state, country, phone_no, email, geo_long, geo_lat, sort_order, status, inserted_on, inserted_by, ip) values ('"+address.name+"','"+address.address+"', '"+address.city+"', '"+address.state+"', '"+address.country+"', '"+address.phone_no+"', '"+address.email+"', '"+address.geo_long+"', '"+address.geo_lat+"', '"+address.sort_order+"', '"+address.status+"', now(), '"+address.inserted_by+"', '"+address.ip+"')");
