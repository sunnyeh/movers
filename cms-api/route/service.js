	...
	app.get('/service',authCheck.ensureAuthorized,function(req,res,next){
		Sync(function(){
			try{
				...
				var sqlCount = "select count(id) as numRows from service where not status=-1"+SQLFilter;
				...
				var sqlList = "select * from service where not status = -1"+SQLFilter+" order by name limit "+SQLlimit;
