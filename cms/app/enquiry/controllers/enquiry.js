...
myApp.controller('EnquiryCtrl', ['$scope','$rootScope','$timeout','enquiryService', function($scope,$rootScope,$timeout,enquiryService) {
	...
	$scope.getEnquiry = function(_page,_search){
		$scope.loader=true;
		$scope.enquiryData={docs:[]};
		var search = '?';
		if(_search){
		...
		enquiryService.getEnquiry(search,$rootScope.confHeader).then(function(success){
			console.log("success",success);
			var Data = success.data;
			if(Data.status){
				$scope.enquiryData = Data.data;
				$scope.enquiryData.docs = $scope.enquiryData.docs.map(function(item){
					item.status=item.status.toString();
					return item;
				});
				console.log("Enquiry",$scope.enquiryData);
